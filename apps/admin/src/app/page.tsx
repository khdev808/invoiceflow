    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
        <div className="bg-white rounded-2xl shadow-lg p-8 w-full max-w-md">
          ...
          <input className="w-full border rounded-lg px-4 py-3 mb-3" placeholder="Email" value={email} onChange={(e) => setEmail(e.target.value)} />
          <input className="w-full border rounded-lg px-4 py-3 mb-4" type="password" placeholder="Password" value={password} onChange={(e) => setPassword(e.target.value)} />
          <button onClick={login} disabled={loading} className="w-full bg-blue-600 text-white rounded-lg py-3 font-semibold hover:bg-blue-700 disabled:opacity-50">
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="p-6 max-w-7xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          {[
            { label: 'Total Users', value: dashboard?.userCount || 0, color: 'blue' },
            { label: 'Total Invoices', value: dashboard?.invoiceCount || 0, color: 'purple' },
            { label: 'Platform Revenue', value: `$${(dashboard?.totalRevenue || 0).toLocaleString()}`, color: 'green' },
            { label: 'Plans Active', value: dashboard?.planBreakdown?.length || 0, color: 'amber' },
          ].map((stat) => (
              ...
              {users.map((u) => (
                <div key={u.id} className="flex items-center justify-between py-2 border-b last:border-0">
                  ...
                  <div className="text-right">
                    <span className="text-xs bg-blue-50 text-blue-700 px-2 py-1 rounded-full font-medium">{u.plan}</span>
            ...
            {dashboard?.monthlySignups?.map((m: any) => (
              <div key={m.month} className="flex items-center gap-3 py-2">
                ...
                <div className="flex-1 bg-slate-100 rounded-full h-2">
                  <div className="bg-blue-600 h-2 rounded-full" style={{ width: `${Math.min(100, m.count * 20)}%` }} />
                </div>
