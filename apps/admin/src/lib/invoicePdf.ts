...
export function buildInvoicePdfHtml(invoice: InvoicePdfData & { showBranding?: boolean }) {
  const tpl = getTemplate(invoice.templateId);
  const render = RENDERERS[tpl.layout] || renderModern;
  const body = render(invoice, tpl);
  const branding = invoice.showBranding
    ? `<div style="text-align:center;padding:16px;font-size:11px;color:#94A3B8;border-top:1px solid #E2E8F0;margin-top:32px">Created with InvoiceFlow · invoiceflow.app</div>`
    : '';
  return `<!DOCTYPE html>
<html><head><meta charset="utf-8"><style>${baseStyles(tpl.layout, tpl.color, tpl.accentColor)}</style></head>
<body>${body}${branding}</body></html>`;
}
